...
// An elder's inbox of pending "link my account as this elder profile" requests sent by a
// family member (see Family/ElderProfiles/LinkElderProfile.page.tsx). Accepting sets this
// account as the profile's elderUserId — see InviteServiceImpl#accept on the backend.
const ElderInvitesPage: React.FC = () => {
  ...
  const respond = async (invite: InviteResponse, action: "accept" | "decline") => {
    ...
    try {
      ...
      showSnackbar("success", action === "accept" ? "You're now linked to this family" : "Invite declined");
      load();
    } catch (err) {
        ...
        <CardContent>
          {loading ? (
            ...
            <EmptyState
              ...
              description="When a family member invites you to link your account as their elder, it will show up here."
              minHeight={220}
            />
          ) : (
...
export default ElderInvitesPage;
